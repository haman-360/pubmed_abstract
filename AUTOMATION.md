...
5. 全件アーカイブ、NotebookLM履歴、固定CURRENTの順にGoogle Docsを作ります。
...
TESTフォルダーだけに、小児腎臓の実データ5論文、一次Batch入力・結果、最終Batch入力・結果、run manifest、全件アーカイブ、NotebookLM履歴、固定CURRENTが作られます。メール件名には`[TEST]`が付きます。
...
- `documents/{topic}/notebooklm_history/`: 最終10本まで、Abstract全文・次点なし
- `documents/{topic}/current/`: NotebookLMへ一度登録する固定文書
