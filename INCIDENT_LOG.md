...
| 日時（JST） | 事象 | 原因 | 影響 | 状態 |
|---|---|---|---|---|
| 2026-09-05 | 土曜定期ダイジェストが届かなかった | workflowの`PMID_LEDGER_WEB_URL`のインデント崩れによりYAMLが無効化され、定期dispatch自体が起動しなかった | 9/5対象の毎週4テーマ・月次3テーマが未検索。論文0件ではなく未実行 | YAML修正。復旧時にJST実行日を指定できるworkflow入力を追加し、`scheduled-2026-09-05`としてBatch経路で再実行予定 |
