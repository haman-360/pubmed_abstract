...
| 日時（JST） | 事象 | 原因 | 影響 | 状態 |
|---|---|---|---|---|
| 2026-09-19 | 定期ダイジェストで小児喘息だけ`FAILED`、新着17件・選定0件・Document `PENDING` | 最終選定の初回応答はPMID `42751894`を2回選定し、自動再試行はPMID `42747714`、`42751182`、`42751907`を選定と次点の両方へ含めた。候補外PMIDはなく、重複検証が2回とも正しく拒否した | 小児喘息17件の選定・配信日別Documentだけが未完了。他テーマへの影響なし | commit `c464476`で`recover-failed`経路、commit `5d52551`で重複を安全に除去する恒久対策をmainへ追加。Actions run `35443124359`で保存済み一次評価から最終Batchだけを復旧投入し、run `35443475710`で完了処理。manifest `COMPLETED`、選定10件・次点5件、配信日別Document作成を確認し、利用者が`[復旧]`メール受信を確認済み |
