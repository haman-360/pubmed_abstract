...
| 日時（JST） | 事象 | 原因 | 影響 | 状態 |
|---|---|---|---|---|
| 2026-09-19 | 定期ダイジェストで小児喘息だけ`FAILED`、新着17件・選定0件・Document `PENDING` | 一次評価と最終BatchのAPI応答は取得済みだが、最終成果物が確定する前に終端失敗となった。正確な`failure_reason`はDrive manifestで確認する | 小児喘息17件の選定・配信日別Documentだけが未完了。他テーマへの影響なし | 保存済み一次評価・候補を再利用して失敗テーマだけを再投入する`recover-failed`経路を追加。main反映後、`scheduled-2026-09-19`の`peds_asthma_update`を復旧投入して結果確認する |
