...
- チェック済み論文へ同じ状態を適用する操作は、状態選択時に未保存の変更として一覧へ反映し、最後のまとめて保存までサーバー通信しない。
...
### 2026-09-02: チェック済み一括変更の操作を明確化

- チェックを付けただけでは「未保存の変更」に数えられず、まとめて保存ボタンが無効のままになる分かりにくさを修正した。
- チェック後に一括状態を選ぶと、対象全件へ画面内で即時適用し、「未保存の変更 N件」としてまとめて保存できるようにした。
- 未保存の変更が0件のときは追従型保存バーを表示せず、状態選択欄や適用ボタンを覆わないようにした。
- 本番Webアプリをバージョン7へ更新し、既存の `/exec` URL、実行者「アクセスしているユーザー」、アクセス範囲「自分のみ」を維持した。
- Node.jsテスト19件、Pythonテスト37件、`git diff --check`に合格し、本番の「原文入手待ち」12件画面で、状態選択後に未保存件数が12件となることを保存なしで確認した。
