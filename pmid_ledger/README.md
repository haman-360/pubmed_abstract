...
- 「原文入手希望に追加」はワンタップ保存です。配信内では各論文の状態と2000文字以内のメモを別々に編集し、末尾の「この配信の変更をまとめて保存」で変更分だけを1回に保存できます。未保存の状態・メモは画面内で保持し、配信一覧へ戻る際やページを離れる際に警告します。個別の「状態・メモを保存」も利用できます。
